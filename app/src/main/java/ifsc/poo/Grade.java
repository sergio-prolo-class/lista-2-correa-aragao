...
public class Grade {
    ...
    public void desenharGrade(){
        ...
        for (int i = 0; i <= altura; i++) {
            ...
            desenho.line(tamanhoCelula * i + 60.0, 60.0, tamanhoCelula * i + 60.0, largura * tamanhoCelula + 60.0); // vertical
        }
        desenharRotulos();
    }

    public void desenharRotulos() {
        desenho.setPenColor(Color.BLACK);
        desenho.setFont(new Font("Arial", Font.PLAIN, 12));
        for (int i = 0; i < rotulosX.size(); i++) {
            desenho.text(60 + tamanhoCelula / 2 + i * tamanhoCelula, 50, rotulosX.get(i)); // ingridy comentar o código.
        }
        for (int i = 0; i < rotulosY.size(); i++) {
            desenho.text(50, 60 + tamanhoCelula / 2 + i * tamanhoCelula, rotulosY.get(i));
        }
    }
